...
func moveToApplicationsFolder() {
    ...
    struct MoveStrings {
        let couldNotMove = NSLocalizedString("Could not move to Applications folder", tableName: "MoveApplication", comment: "")
        let questionTitle = NSLocalizedString("Move to Applications folder?", tableName: "MoveApplication", comment: "")
        let questionTitleHome = NSLocalizedString("Move to Applications folder in your Home folder?", tableName: "MoveApplication", comment: "")
        let questionMessage = NSLocalizedString("I can move myself to the Applications folder if you'd like.", tableName: "MoveApplication", comment: "")
        let buttonMove = NSLocalizedString("Move to Applications Folder", tableName: "MoveApplication", comment: "")
...
//    let destinationAbs = destinationURL.absoluteURL
...
fileprivate func PreferredInstallLocation() -> (URL?, Bool) {
    ...
    if let userDir = userAppDirs.first {
        ...
        if fm.fileExists(atPath: userDir, isDirectory: &directory ) {
...
func copyBundle(srcPath: String, dstPath: String) -> Bool {
    ...
    do {
        ...
        print("ERROR -- Could not copy '%@' to '%@' (%@)", srcPath, dstPath, error.localizedDescription)
        NSAlert(error: error).runModal()
        return false
    }
}
